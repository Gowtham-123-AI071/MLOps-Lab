...
for user_id in test_users:
    ...
    for sim_user_id, score in similar:
        print(f"  User {int(sim_user_id)}: similarity = {float(score):.4f}")
...
for user_id in [1, 2, 3]:
    vec = features.get_user_ratings_vector(user_id)
    rated = np.sum(vec > 0)
    avg = np.mean(vec[vec > 0])
    print(f"User {user_id}: rated {rated} movies, avg rating = {avg:.2f}")
